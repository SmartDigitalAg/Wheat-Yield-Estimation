...
def get_satellite_df(start_date, end_date, site, img_key, imgs, vi_lst, cloud_filter):
    ...
    filtered_imgs = imgs.filterDate(start_date, end_date).filterBounds(site).map(lambda image: mask_clouds(image, img_key))

    if cloud_filter:
        filterd_imgs = (filtered_imgs.filter(ee.Filter.lt(cloud_percentage_band, 20)))
    else:
        filterd_imgs = filtered_imgs

    processed_img_collection = (filterd_imgs
...
def main():
    input_dir, output_dir = make_dir('./input'), make_dir('./output')
    ...
    check_points = ['cloud', 'resampled']
    for check_point in check_points:
        output_filename = os.path.join(output_dir, f'satellite_{check_point}.csv')
