...
def preprocess_drone(drone_filename):
...
def generate_data(growth_filename, drone_filename):
    tillering_pro = preprocess_tillering_pro(growth_filename)
    tillering_telo = preprocess_tillering_telo(growth_filename)
    flowering1 = preprocess_flowering1(growth_filename)
    flowering2 = preprocess_flowering2(growth_filename)
    flowering4 = preprocess_flowering4(growth_filename)
    harvesting = preprocess_harvesting(growth_filename)
    harvesting = harvesting[['반복', '조사지', '수량(g/m2)_수확기']]

    drone = preprocess_drone(drone_filename)
...
def main():
    input_dir = './input'
    data_dir = os.path.join(input_dir, '2023')

    growth_filename = os.path.join(data_dir, '생육조사결과.xlsx')
    drone_filename = os.path.join(data_dir, '샘플링위치별_식생지수및수확량.xlsx')

    output_filename = os.path.join(input_dir, '2023_data.csv')

    df_all = generate_data(growth_filename, drone_filename)
